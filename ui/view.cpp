...
void View::initializeGL() {
    ...
    // World setup
    m_world = std::make_shared<DemoWorld>();
    m_world->makeCurrent();
}
...
void View::keyPressEvent(QKeyEvent *event) {
    ...
    // Wave commands
    if (event->key() == Qt::Key_R) m_world->reset();
    if (event->key() == Qt::Key_I) m_world->saveInstantGesture();
